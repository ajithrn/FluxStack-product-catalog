...
The plugin uses a simple npm-based build step to concatenate and minify static assets. Source files are in `assets/css/` and `assets/js/`, built output goes to `assets/dist/`.
...
| Command | Description |
|---------|-------------|
| `npm run build` | Build all CSS and JS (production) |
...
| File | Contents |
|------|----------|
| `assets/dist/frontend.min.css` | All frontend CSS (common + archive + single) |
| `assets/dist/frontend.min.js` | All frontend JS (archive + single) |
| `assets/dist/admin.min.css` | All admin CSS (admin + settings) |
| `assets/dist/admin-settings.min.js` | Admin settings JS |

### How It Works

- The plugin always loads from `assets/dist/` — the build step is **required**
- Source files in `assets/css/` and `assets/js/` are for development only
- One CSS + one JS request per page type (frontend or admin)
- No webpack/vite — just `clean-css-cli` and `terser` via npm scripts

### After Editing Source Files

Always run `npm run build` after modifying any CSS or JS source file. Use `npm run watch` during active development to auto-rebuild on save.
